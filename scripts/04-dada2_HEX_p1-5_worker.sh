#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# DADA2 HEX — Job 1 : filterAndTrim + learnErrors + dada, plaques 1-5
...
#   err_fwd_HEX.rds       — modèle d'erreur forward (partagé avec job 2)
#   err_rev_HEX.rds       — modèle d'erreur reverse (partagé avec job 2)
#   seqtab_HEX_P1-5.rds   — table ASVs plaques 1-5
# ─────────────────────────────────────────────────────────────────────────────
...
export R_DEMUX_DIR="$PROJECT_DIR/results/cutadapt_HEX_4258571/demux"
...
saveRDS(err_fwd, file.path(shared_dir, "err_fwd_HEX.rds"))
saveRDS(err_rev, file.path(shared_dir, "err_rev_HEX.rds"))
...
saveRDS(seqtab_P1_5, file.path(shared_dir, "seqtab_HEX_P1-5.rds"))
cat(sprintf("✓ seqtab_HEX_P1-5.rds sauvegardé dans %s\n", shared_dir))
...
write.csv(track, file.path(shared_dir, "track_HEX_P1-5.csv"), row.names=FALSE)
