#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# DADA2 HCO — Job 3 : mergeSequenceTables + removeBimeraDenovo
# Ne pas lancer directement — soumis par 04-dada2_HCO_launcher.sh
# Variables reçues via --export : SHARED_DIR
#
# Produit le résultat final : seqtab_HCO_final.rds + seqtab_HCO_final.csv
# ─────────────────────────────────────────────────────────────────────────────
...
cat("=== mergeSequenceTables + removeBimeraDenovo — HCO ===\n\n")

# ── Charger les deux seqtabs ──────────────────────────────────────────────
f_P1_5 <- file.path(shared_dir, "seqtab_HCO_P1-5.rds")
f_P6   <- file.path(shared_dir, "seqtab_HCO_P6.rds")
...
track_p1_5 <- read.csv(file.path(shared_dir, "track_HCO_P1-5.csv"))
track_p6   <- read.csv(file.path(shared_dir, "track_HCO_P6.csv"))
...
saveRDS(seqtab_nochim,
        file.path(scratch, "seqtab_HCO_final.rds"))
write.csv(t(seqtab_nochim),
          file.path(scratch, "seqtab_HCO_final.csv"))
write.csv(track_all,
          file.path(scratch, "track_HCO_final.csv"), row.names=FALSE)

cat(sprintf("\n✓ Fichiers finaux dans %s :\n", scratch))
cat("  seqtab_HCO_final.rds   — pour analyses R aval\n")
cat("  seqtab_HCO_final.csv   — ASVs en lignes, samples en colonnes\n")
cat("  track_HCO_final.csv    — suivi des reads par sample à chaque étape\n")
EOF

# ─────────────────────────────────────────────────────────────────────────────
rsync -av "$RUN_SCRATCH/" "$RUN_RESULTS/"
echo "✓ Résultats finaux → $RUN_RESULTS"
echo "  seqtab_HCO_final.rds : $RUN_RESULTS/seqtab_HCO_final.rds"
