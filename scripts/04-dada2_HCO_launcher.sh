...
# ─────────────────────────────────────────────────────────────────────────────
# DADA2 HCO — Launcher
# Usage : bash scripts/04-dada2_HCO_launcher.sh
#
# Architecture :
...
SHARED_DIR="$PROJECT_DIR/results/dada2_HCO_shared"
mkdir -p "$SHARED_DIR"
echo "Dossier partagé : $SHARED_DIR"
echo ""

echo "=== Pipeline DADA2 — marqueur HCO ==="
echo ""

# ── Job 1 : plaques 1-5 ───────────────────────────────────────────────────────
JID1=$(sbatch --parsable \
    --job-name="dada2_HCO_p1-5" \
    --export=ALL,SHARED_DIR="$SHARED_DIR" \
    "$SCRIPTS_DIR/04-dada2_HCO_p1-5_worker.sh")
echo "→ Job 1 soumis (plaques 1-5) : $JID1"

# ── Job 2 : plaque 6 (attend job 1 pour le modèle d'erreur) ──────────────────
JID2=$(sbatch --parsable \
    --dependency=afterok:"$JID1" \
    --job-name="dada2_HCO_p6" \
    --export=ALL,SHARED_DIR="$SHARED_DIR" \
    "$SCRIPTS_DIR/04-dada2_HCO_p6_worker.sh")
...
JID3=$(sbatch --parsable \
    ...
    --job-name="dada2_HCO_final" \
    --export=ALL,SHARED_DIR="$SHARED_DIR" \
    "$SCRIPTS_DIR/04-dada2_HCO_final_worker.sh")
echo "→ Job 3 soumis (final)       : $JID3 (attend $JID1 et $JID2)"

echo ""
echo "✓ Pipeline HCO soumis."
echo ""
