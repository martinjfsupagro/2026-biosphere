#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# DADA2 HCO — Job 1 : filterAndTrim + learnErrors + dada, plaques 1-5
...
#   err_fwd_HCO.rds       — modèle d'erreur forward (partagé avec job 2)
#   err_rev_HCO.rds       — modèle d'erreur reverse (partagé avec job 2)
#   seqtab_HCO_P1-5.rds   — table ASVs plaques 1-5
# ─────────────────────────────────────────────────────────────────────────────
...
export R_DEMUX_DIR="$PROJECT_DIR/results/cutadapt_HCO_4258572/demux"
...
saveRDS(err_fwd, file.path(shared_dir, "err_fwd_HCO.rds"))
saveRDS(err_rev, file.path(shared_dir, "err_rev_HCO.rds"))
...
saveRDS(seqtab_P1_5, file.path(shared_dir, "seqtab_HCO_P1-5.rds"))
cat(sprintf("✓ seqtab_HCO_P1-5.rds sauvegardé dans %s\n", shared_dir))
...
write.csv(track, file.path(shared_dir, "track_HCO_P1-5.csv"), row.names=FALSE)
