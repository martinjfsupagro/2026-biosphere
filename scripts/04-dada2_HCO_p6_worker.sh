#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# DADA2 HCO — Job 2 : filterAndTrim + dada, plaque 6
# Ne pas lancer directement — soumis par 04-dada2_HCO_launcher.sh
# Variables reçues via --export : SHARED_DIR
#
# Attend job 1 (plaque 1-5) pour charger err_fwd/rev_HCO.rds
...
err_fwd_path <- file.path(shared_dir, "err_fwd_HCO.rds")
err_rev_path <- file.path(shared_dir, "err_rev_HCO.rds")
...
saveRDS(seqtab_P6, file.path(shared_dir, "seqtab_HCO_P6.rds"))
cat(sprintf("\n✓ seqtab_HCO_P6.rds sauvegardé dans %s\n", shared_dir))
...
write.csv(track, file.path(shared_dir, "track_HCO_P6.csv"), row.names=FALSE)
