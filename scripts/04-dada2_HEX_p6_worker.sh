#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# DADA2 HEX — Job 2 : filterAndTrim + dada, plaque 6
# Ne pas lancer directement — soumis par 04-dada2_HEX_launcher.sh
# Variables reçues via --export : SHARED_DIR
#
# Attend job 1 (plaque 1-5) pour charger err_fwd/rev_HEX.rds
...
err_fwd_path <- file.path(shared_dir, "err_fwd_HEX.rds")
err_rev_path <- file.path(shared_dir, "err_rev_HEX.rds")
...
saveRDS(seqtab_P6, file.path(shared_dir, "seqtab_HEX_P6.rds"))
cat(sprintf("\n✓ seqtab_HEX_P6.rds sauvegardé dans %s\n", shared_dir))
...
write.csv(track, file.path(shared_dir, "track_HEX_P6.csv"), row.names=FALSE)
